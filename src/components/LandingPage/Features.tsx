...
export function Features() {
  const features = [
    {
      icon: Sparkles,
      badge: { text: "Live", color: "bg-green-500 text-white", style: {} },
    ...
    {
      icon: FileText,
      badge: { text: "Q1 2026", color: "text-white", style: { backgroundColor: '#1A2E40' } },
  ...
  return (
    <section id="services" className="py-20 lg:py-24 bg-gradient-to-b from-white to-gray-50">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {features.map((feature, index) => {
            ...
            return (
              <div
                key={index}
                className="group relative bg-white border border-gray-200 rounded-2xl p-8 hover:-translate-y-2 hover:shadow-xl transition-all duration-300"
              >
                {/* Badge */}
                <div
                  className={`absolute top-6 right-6 ${feature.badge.color} text-xs font-medium px-3 py-1 rounded-full`}
                  style={feature.badge.style}
                >
                  {feature.badge.text}
                </div>

                {/* Icon */}
                <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-[#F29224] to-orange-600 flex items-center justify-center mb-6">
